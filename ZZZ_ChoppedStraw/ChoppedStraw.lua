...
--
-- Edit by Decker_MMIV (@date: 2015-06-23)
...
function ChoppedStraw:updateTick(dt)
	...
	then
		...
		and fruitDesc ~= nil
		and fruitDesc.index == FruitUtil.FRUITTYPE_MAIZE
		then
			local preparingOutputId = g_currentMission.fruits[FruitUtil.FRUITTYPE_CHOPPEDMAIZE].preparingOutputId;
		...
		elseif self.chopperPSenabled then
			...
			if fruitDesc.index == FruitUtil.FRUITTYPE_RAPE then
				preparingOutputId = g_currentMission.fruits[FruitUtil.FRUITTYPE_CHOPPEDRAPE].preparingOutputId;
			elseif fruitDesc.index ~= FruitUtil.FRUITTYPE_MAIZE then
